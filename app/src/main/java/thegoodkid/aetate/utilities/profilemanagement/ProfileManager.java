...
import static thegoodkid.aetate.utilities.Reporter.LOG_D;
import static thegoodkid.aetate.utilities.Reporter.LOG_I;
import static thegoodkid.aetate.utilities.Reporter.LOG_V;
import static thegoodkid.aetate.utilities.Reporter.LOG_W;
import static thegoodkid.aetate.utilities.Reporter.reportError;

public class ProfileManager implements ProfileManagerInterface.OnProfileUpdatedListener, TagManager.OnTaggedListener {
    private static final String PREFERENCE_KEY = "thegoodkid.aetate.pref.PROFILEMANAGER";
    ...
    private static final String JSON_TREE_PROFILES_KEY = "thegoodkid.aetate.pref.PROFILEMANAGER.JSONPROFILES";
    ...
    private ArrayList<ProfileManagerInterface.OnProfileAddedInBatchListener> mOnBatchAddedListener = new ArrayList<>();
        ...
        if (instance == null) {
            ...
            if (instance.mPreference.contains(JSON_TREE_PROFILES_KEY)) {
                try {
                    ...
                    int size = instance.mJsonProfiles.length();
                    for (int i = 0; i < size; i++) {
                        ...
                        Avatar avatar = null;
                        if (jsonProfile.has(Profile.KEY_AVATAR_NAME)) {
                            avatar = Avatar.retrieveAvatar(context, jsonProfile.getString(Profile.KEY_AVATAR_NAME));
                            if (LOG_I)
    ...
    private boolean addProfileSilent(@NonNull Profile profile) {
    ...
    public void addProfile(Profile profile) {
        ...
        if (!addProfileSilent(profile)) return;
    ...
    public void addProfiles(List<Profile> profiles) {
        for (Profile profile : profiles) {
            addProfileSilent(profile);
        }

        updatePreference();
        triggerBatchAdded(profiles);
    }
    ...
    public void onProfileAvatarChanged(int profileId, Avatar newAvatar, Avatar previousAvatar) {
        ...
        if (object == null) {
            ...
            if (profile == null) {
                reportError(LOG_TAG, Error.createDevMessage(Error.INCONSISTENT_MODIFIED_ABSENT));
                return;
            }

            object = profile.toJSONObject();
            mJsonProfiles.put(object);
        }

        try {
            object.put(Profile.KEY_AVATAR_NAME, newAvatar.getAvatarFileName());
        } catch (JSONException e) {
            e.printStackTrace();
        }
    ...
    public ArrayList<Profile> getPinnedProfiles() {
        ...
        for (int id : ids) {
            ...
            if (profile != null) profiles.add(profile);
                ...
                if (LOG_W) Log.e(LOG_TAG, Error.createDevMessage(Error.INCONSISTENT_TAGGED_ABSENT));
                mTagManager.prepareProfileForRemove(id);
            }
        }
    ...
    public void addOnProfileAddedInBatchListener(ProfileManagerInterface.OnProfileAddedInBatchListener listener) {
        mOnBatchAddedListener.add(listener);
    }

    public void removeOnProfileAddedInBatchListener(ProfileManagerInterface.OnProfileAddedInBatchListener listener) {
        mOnBatchAddedListener.remove(listener);
    }
    ...
    private void triggerBatchAdded(List<Profile> profiles) {
        boolean needsCleanup = false;

        for (ProfileManagerInterface.OnProfileAddedInBatchListener listener : mOnBatchAddedListener) {
            if (listener != null) listener.onProfileAddedInBatch(profiles);
            else needsCleanup = true;
        }
